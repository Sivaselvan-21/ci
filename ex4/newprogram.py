...
def entropy(rows, target):
    """Entropy H(S) for the target column."""
    label_counts = {}
    for row in rows:
        y = row[target]
...
def split_by_value(rows, attr):
    """Partition rows by attribute value."""
    groups = {}
    for row in rows:
        v = row[attr]
...
def info_gain(rows, attr, target):
    """IG(S, attr) = H(S) - sum_v (|S_v|/|S|) * H(S_v)."""
...
def step_by_step_report(rows, attributes, target):
    """Print step-by-step entropy & IG, then summary with best root attribute."""
    n = len(rows)
...
def load_dataset_from_txt(filename):
    """Load dataset from CSV-like .txt file."""
    data = []
    with open(filename, "r") as f:
        lines = f.read().strip().splitlines()

    headers = lines[0].split(",")   # first line is header
    for line in lines[1:]:
        values = line.split(",")
        row = {headers[i]: values[i] for i in range(len(headers))}
        data.append(row)

    return data, headers[:-1], headers[-1]   # attributes, target
